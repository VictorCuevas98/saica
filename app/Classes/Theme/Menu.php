...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        //LINEA AGREDA POR NOE PARA CHECAR ROLES Y PERMISOS
        $autorizado = self::checkAutorizeRoleOrPermission($item);
        if(!$autorizado)
            return ;
        //TERMINA LINEAS  AGREDA POR NOE PARA CHECAR ROLES Y PERMISOS
    ...
    public static function checkAutorizeRoleOrPermission($item){
        $roles = self::explodeItemStringToArray($item,"roles" );
        $permissions = self::explodeItemStringToArray($item,'permissions');
        $authorize = false;
        
        if(count($roles)>0 && count($permissions)>0){
            //hay roles y permisos para el menu
            if(Auth::user()->hasAnyRole($roles) || Auth::user()->hasAnyPermission($permissions) ){
                $authorize = true;
            }
        }else if( count($roles)>0 && count($permissions)<=0 ){
            //revisar solo roles
            if(Auth::user()->hasAnyRole($roles)  ){
                $authorize = true;
            }
        }else if( count($roles)<=0 && count($permissions)>0 ){
            //revisar solo permisos
            if(Auth::user()->hasAnyPermission($permissions)  ){
                $authorize = true;
            }
            
        }else{
            //No hay reglas, permitir 
            $authorize = true;
        }
        return $authorize;
    }
    public static function explodeItemStringToArray($item,$itemName ){
        $cadena = isset($item[$itemName]) ? $item[$itemName] : [];
        //dd($cadena);
        $arregloFinal = is_array($cadena)
            ? $cadena
            : explode('|', $cadena);
        return $arregloFinal;
    }
