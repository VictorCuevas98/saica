...
namespace App\Http\Controllers\Entradas\ContratosCerrados;
...
class EntradaContratoCerradoArticulosController extends Controller
{
    //
    public function index($adquisicionId,$entradaId)
    {

        if(!Auth::user()->hasAnyPermission(['entradas.contratosCerrados.articulos.listar']))
        ...
        //BUSCAMOS LOS TIPOS DE CONTRATOS DE LA ADQUISICION
        $adquisicionTiposContrato = $adquisicion->contratos()->with('tipoContrato')->get()->pluck('tipoContrato.clave_tipo_contrato');

        if($adquisicionTiposContrato->contains('C')){
            //LA ADQUISICION TIENE CONTRATOS CERRADOS 
            ...
            return view('entradas.contratoCerrado.articulos.articulosIndex',compact('adquisicionId','adquisicion','entradaId','articulos'));
        }else{
            abort(404,'Lo sentimos pero la adquiscion no contiene contratos cerrados');
        }
    ...
    public function create(Request $request,$adquisicionId,$entradaId ){
        ...
        return view('entradas.contratoCerrado.articulos.articulo_new_modal',compact('articulo','catLaboratorio','adquisicionId','adquisicion','entradaId'))->render();
    }
    ...
    public function edit(Request $request,$adquisicionId,$entradaId ,$articuloId){
        ...
        return view('entradas.contratoCerrado.articulos.articulo_edit_modal',compact('catLaboratorio','adquisicionId','adquisicion','entradaId','entradaAdquisicionDetalle','articuloId','articulo'))->render();
    }
